...
| defaultSettings.dataEngineIPFamily | Preferred IP family shared by Longhorn V1/V2 data-engine and backing-image management traffic. The default is empty (""). Allowed values are exactly "", "ipv4", and "ipv6"; null is unsupported. Empty preserves legacy node-default address selection. An explicit family is strict and never falls back to the other family or another network. Change this setting only after all volumes are detached because BIM, BIDS, and instance-manager pods must restart. |
