...
| defaultSettings.orphanResourceAutoDeletion | Setting that allows Longhorn to automatically delete an orphaned resource and the corresponding data or process (for example, stale replicas). Orphaned resources on failed or unknown nodes are not automatically cleaned up. Specify enabled resource types using a semicolon-separated list (Example: `replicaData;replicaInstance`). Available items are: `replicaData`, `engineInstance`, `replicaInstance`. |
