...
| longhornManager.resources | object | `nil` | Resource requests and limits for Longhorn Manager pods. |
...
| defaultSettings.systemManagedCSIComponentsResourceLimits | This setting allows you to configure CPU and memory requests/limits for system-managed CSI components. Supported components include: `csi-attacher`, `csi-provisioner`, `csi-resizer`, `csi-snapshotter`, `longhorn-csi-plugin`, `node-driver-registrar`, and `longhorn-liveness-probe`. Only the components defined in the JSON object will have their resource requirements overridden; others will continue using Longhorn’s defaults. Updating resource limits will restart the affected CSI components, which may temporarily delay new volume provisioning, expansion, snapshot, or attach/detach operations. The value must be a JSON object with component names as keys and Kubernetes ResourceRequirements (requests and limits). |
