...
| Key | Description |
|-----|-------------|
| service.ipFamilyPolicy | IP family policy for Longhorn Services. Leave empty to preserve the current Kubernetes default. (Options: "", "SingleStack", "PreferDualStack", "RequireDualStack") |
...
| defaultSettings.preferredDataEngineIPFamily | Preferred IP family for Longhorn data-engine and backing-image management traffic. The default value (`default`) preserves legacy node-default address selection. Allowed values are exactly `default`, `ipv4`, and `ipv6`; null and empty values are unsupported. An explicit family is strict and never falls back to the other family or another network. **Danger Zone**: Change this setting only after all volumes are detached. Backing Image Manager (BIM) and Backing Image Data Source (BIDS) pods restart to apply it, while capable V2 Instance Manager pods remain running. You cannot modify this setting while volumes are still attached. |
