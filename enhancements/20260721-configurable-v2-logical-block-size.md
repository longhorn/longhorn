...
- [V2 sharding](20260519-v2-sharding.md)
...
`Volume.spec.logicalBlockSize` remains top-level because it is a frontend and data-format property shared by both `dataLayout.type: replicated` and `dataLayout.type: sharded`. The data layout selects redundancy and topology, while every `Replica` or `Shard` placement must satisfy the same volume block-size contract.
...
For sharded volumes, these propagation rules apply when the corresponding workflow is supported; this enhancement does not enable clone, backup/restore, or DR for sharded volumes.
...
The existing disk-service `DiskCreate` RPC already carries a block size and reports the effective size. Engine, replica, rebuild, and `ShardGroup` create requests gain an expected logical block size so the data plane can validate connected bdevs against the Volume contract rather than merely checking that they match each other.
...
5. If any relevant placement is unknown or reports an unsupported value, or if multiple values exist, leave the field unresolved and set a condition describing the conflict.
...
The manager passes the immutable expected logical block size through engine, replica, rebuild, and `ShardGroup` requests. The data plane validates the invariant at its trust boundary:
