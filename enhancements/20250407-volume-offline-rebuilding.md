...
Introduce new volume Action APIs `offlineRebuilding` and a new field `Volume.Spec.OfflineRebuild`:

  | API | Input | Output | Comments | HTTP Endpoint |
  | --- | --- | --- | --- | --- |
  | Update | N/A | err error | Enable/Disable volume offline rebuilding | **POST** `/v1/volumes/{VolumeName}?action=offlineRebuilding` |
...
|| `true` | `false` | Global Setting </br>`offline-replica-rebuilding` |
| ---: | :---: | :---: | --- |
| `Volume.Spec.OfflineRebuild` `ignored` | ✓ | X ||
| `enabled` | ✓ | ✓ ||
| `disabled` | X | X ||
...
- By the Longhorn UI
  ...
  3. Click on the `Operation` dropdown and click on `Offline Replica Rebuild`.
  4. A UI lightbox will pop out and users choose the option `enabled`.

  Three options for the `Offline Replica Rebuild` (`Volume.Spec.OfflineRebuild`):
  - `ignored`: The default value. When the `Volume.Spec.OfflineRebuild` is `ignored`, the offline rebuilding will follow the value of the global setting `offline-replica-rebuilding`.
...
- By `kubectl` command:
  ...
  2. Update the field `Volume.spec.offlineRebuild` to `enabled`.
...
- The `Volume.Spec.OfflineRebuild` field will `NOT` be changed when the global setting `offline-replica-rebuilding` is modified.

#### Global Setting `offline-replica-rebuilding`

- The setting `offline-replica-rebuilding` is `true`:
  1. The volume rebuild controller will create the VA ticket for attaching the volume that the `Spec.OfflineRebuild` field is `enabled` or `ignored` and need to be rebuilt.
...
- The setting `offline-replica-rebuilding` is `false`:
  1. The volume rebuild controller will check if the `Volume.Spec.OfflineRebuild` field is `disabled` or `ignored`.
...
- The `Spec.OfflineRebuild` field of the detached volume is set to `enabled` by the Longhorn UI, API or `kubectl` command:
...
- The `Spec.OfflineRebuild` field of the detached volume is set to `disabled` by the Longhorn UI, API or `kubectl` command:
...
#### CSI Attach Request During Offline Rebuilding
...
- The volume rebuild controller will examine all volumes:
  - If `Volume.Spec.OfflineRebuild` is `disabled`, remove the volume rebuild VA ticket for the volume.
  - If `Volume.Spec.OfflineRebuild` is `enabled` or if the global setting `offline-replica-rebuilding` is `true` and the `Volume.Spec.OfflineRebuild` field is `ignored`, the volume needs to be rebuilt and there is no CSI VA ticket:
...
- The replica count is less than the number of replicas in the `Volume.Spec`:
  - Set the field `Volume.Spec.OfflineRebuild` to `enabled`
    ...
    4. Enable the offline rebuilding by the API `volume.offlineRebuilding`.
    5. Wait for the volume detached.
    6. Check if healthy replicas count of the volume equals to `Volume.Spec.NumberOfReplicas`.
    7. Check if the `Volume.Spec.OfflineRebuild` is still `enabled`.
    ...
    12. Check if the `Volume.Spec.OfflineRebuild` is still `enabled`.
  ...
  - Set the global setting `offline-replica-rebuilding` to `true`
    ...
    5. Wait for the volume detached.
    6. Check if healthy replicas count of the volume equals to `Volume.Spec.NumberOfReplicas`.
    7. Check if the `Volume.Spec.OfflineRebuild` is still `ignored`.
    8. Delete a replica of the volume.
    9. The offline rebuilding will start again.
    10. Wait for the volume detached.
    11. Check if healthy replicas count of the volume equals to `Volume.Spec.NumberOfReplicas`.
    12. Check if the `Volume.Spec.OfflineRebuild` is still `ignored`.
    13. Set the global setting `offline-replica-rebuilding` to `false`.
    14. Check if the `Volume.Spec.OfflineRebuild` is `ignored`
    15. Set the `Volume.Spec.OfflineRebuild` to `disable`.
    ...
    19. Check if the `Volume.Spec.OfflineRebuild` is still `disable`.
    20. Set the global setting `offline-replica-rebuilding` to `false`.
    21. Check if the `Volume.Spec.OfflineRebuild` is still `disable`.
...
- The CSI attaching request should preempt the offline rebuilding process:
  ...
  5. Enable the offline rebuilding by the API `volume.offlineRebuilding`.
...
- The `Volume.Spec.OfflineRebuild` field of all volumes will be updated to `ignored`.
