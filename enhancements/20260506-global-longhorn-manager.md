...
`longhorn-manager` runs as a DaemonSet — one pod per node, each
hosting every Longhorn control-plane reconciler alongside the
node-local work (replica/engine/instance-manager lifecycle, disk
monitoring, environment checks). As clusters have grown, two
...
controllers driving them are owner-elected — only one daemon pod
does useful work at a time; the other N − 1 carry the watch and
cache cost just to be ready for failover.
...
to namespace-scoped (`longhorn-system`).

This release **focuses on the cluster-wide Pod informer consumers**
— `KubernetesPVController` and `KubernetesPodController`. The
framework can host additional controllers later if a specific case
justifies it, but no further migration is committed by this
enhancement.

The change is **opt-in and default-off**: chart toggle
`globalManager.enabled` defaults to `false`, so existing operators
see no behavioral change on upgrade.
...
Lease loss or process termination follows standard leader-election
behavior — the lease is released so a standby takes over without
waiting out the full lease duration.

### Failure modes

Until a leader is elected (initial startup or mid-failover),
reconcile for the migrated controllers pauses: force-delete on
node-down, RWX remount, force-delete cleanup, and
`Volume.Status.KubernetesStatus` tracking are all deferred. All
other Longhorn controllers — including the data plane (engine,
replica, instance-manager) — continue running in the DaemonSet, so
in-flight volume I/O is unaffected.

Network partitions between the global manager and a node reuse the
existing `IsNodeDownOrDeletedOrMissingManager` path: a partitioned
node looks down to the global manager and the usual down-node
behaviors trigger as designed. No new partition semantics are
introduced.
