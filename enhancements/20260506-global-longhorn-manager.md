...
Both costs grow with the cluster even though reconciliation is distributed
by volume ownership or node identity: every daemon pod still carries the
cluster-wide Pod watch and cache.
...
global-manager replica (a small constant, e.g. 3).
...
- `longhornGlobalManager.replicas` (default 3) — one active leader plus warm
  standbys via leader election.
- `priorityClass`, `affinity`, `resources`, `tolerations`, `nodeSelector`,
  and pod annotations for the Deployment, plus an opt-in
  PodDisruptionBudget.
...
lifecycle cleanly (controller execution is gated by the Lease) and runs without
...
already-synced cache without a fresh initial cluster-wide LIST. Takeover
still depends on Lease release or expiry and acquisition.
...
During the DaemonSet rollout, old manager pods and the global manager may
both update `Volume.Status.KubernetesStatus`. The existing reconciliation
logic and optimistic-concurrency retry are retained. No CR `Status.OwnerID`
flip is involved — the hosted controllers do not own a Longhorn CR.

Rollback is symmetric: a chart/image rollback removes the Deployment and the
DaemonSet resumes hosting the two controllers, with the same reconciliation
logic during the transition. There is no CRD, on-disk format, or REST
...
During a leader transition there is a gap while a new leader acquires the
Lease, in which the hosted
...
unaffected. The new leader reconciles current state. PV deletion cleanup
for retained Longhorn volumes still depends on delete events and can be
missed during failover; this existing limitation is tracked in
https://github.com/longhorn/longhorn/issues/13654.
...
- **Leader ↔ `kube-apiserver`.** The leader can no longer renew the Lease
  ...
  Lease and takes over using its warm cache after Lease release or expiry.
...
standbys avoid an initial cache sync after acquiring the Lease.
...
- **Small and single-node clusters.** Hostname anti-affinity is
  `preferred`, not `required`, so the configured replicas (default 3) can
  run on the same node. Set `longhornGlobalManager.replicas: 1` to run one.
...
- **Observability.** Leader identity is available in the Lease and logs.
  DaemonSet managers log warnings for a missing or stale global-manager
  Lease, or one released without a successor. Readiness and liveness probes
  check process liveness, not leadership or cache-sync completion.
