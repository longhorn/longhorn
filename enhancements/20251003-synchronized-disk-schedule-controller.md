...
type DiskScheduleSpec struct {
    ...
    // +optional
    Replicas map[string]int64 `json:"schedulingReplicas"`
    // Scheduling requests for backing images. Key is backing image name, value is required size in bytes.
    // +optional
    BackingImages map[string]int64 `json:"schedulingBackingImages"`
}
...
type DiskScheduleStatus struct {
    ...
    // +optional
    Replicas map[string]*DiskScheduledResourcesStatus `json:"scheduledReplicaStatus"`
    // Scheduled status of backing images. Key is backing image name.
    // +optional
    BackingImages map[string]*DiskScheduledResourcesStatus `json:"scheduledBackingImageStatus"`
...
- A resource recorded in `Replicas` (spec JSON: `schedulingReplicas`) or `BackingImages` (spec JSON: `schedulingBackingImages`) of a DiskSchedule's spec that is acquiring space, and
- A resource not yet recorded in `Replicas` (status JSON: `scheduledReplicaStatus`) or `BackingImages` (status JSON: `scheduledBackingImageStatus`) of a DiskSchedule's status, or whose scheduled size is less than its required space.
