...
- No Synchronization: The replica scheduler is stateless, querying current Node/Replica CR status without any locking mechanism
...
Introduce a synchronous Disk Scheduling Controller that records allocated disk resources in Kubernetes Custom Resources (CRs). The DiskSchedule controller manages DiskSchedule CR as the single source of truth for disk allocation state of a disk. All controllers must allocate disk space via this DiskSchedule CR before instantiating replicas or backing image copies on a disk. The controller synchronizes allocation operations on the DiskSchedule CR using Kubernetes optimistic concurrency (resource version conflicts trigger requeue), and the DiskSchedule controller reconciles allocation states in the DiskSchedule CR, ensuring atomicity at the Kubernetes Custom Resource API level.
...
- **DiskSchedule CRD**: A new Custom Resource Definition to store disk scheduling state, including requested and allocated resources for replicas and backing images. Each disk on each node has a corresponding DiskSchedule CR, and are managed by the DiskScheduleController of the owner of the node.
- **DiskScheduleController**: Maintains disk allocation status and decides whether to allocate space for resources. Reconciles `DiskSchedule.Status` based on `DiskSchedule.Spec` allocation requests. Each node has one DiskSchedule controller instance that manages all disks on the node. Mutex is not required as the controller is the single source of truth for disk allocation state.
...
- **State management**: The disk space scheduling logic is computed from `DiskSchedule.Spec` and `Node.Status.DiskStatus`, and recorded in `DiskSchedule.Status`.
...
type DiskScheduleStatus struct {
    ...
    // Sum of successfully scheduled storage in bytes. Should not exceed overProvisionLimit:
    // overProvisionLimit = (node.status.diskStatus[diskName].storageMaximum - node.spec.diskSpec[diskName].storageReserved) * (overProvisioningPercentage) / 100)
    // +optional
...
### DiskSchedule CR Lifecycle

A DiskSchedule CR is created when a disk is added to a node, and deleted when the disk is removed from the cluster. The Node Controller is responsible for managing the lifecycle of DiskSchedule CRs.

### Disk Space Allocation

A resource to be scheduled is defined as:

- A resource recorded in `spec.Replicas` (spec JSON: `replicas`) or `spec.BackingImages` (spec JSON: `backingImages`) of a DiskSchedule's spec that is acquiring space, and
- A resource not yet recorded in `status.Replicas` (status JSON: `replicas`) or `status.BackingImages` (status JSON: `backingImages`) of a DiskSchedule's status, or whose scheduled size is less than its required space.

Before the scheduling controller allocates space to any resource, it first collects the maximum schedulable space from `Node` CR. After each DiskSchedule reconciliation, the total allocated resource size must not exceed this value.

$$overProvisionLimit = (node.status.diskStatus[diskName].storageMaximum - node.spec.diskSpec[diskName].storageReserved) * (overProvisioningPercentage) / 100)$$

$$storageScheduled = \sum(Replicas) + \sum(BackingImages)$$

$$storageScheduled \le overProvisionLimit$$
...
sequenceDiagram
    ...
    participant R as ReplicaCR
    participant DS as DiskScheduleCR
    participant DSC as DiskScheduleController
    participant RC as ReplicaController

    VC->>R: Create Replica (nodeID="", diskID="")

    VC->>VC: ReplicaScheduler picks a disk
    VC->>R: Assign nodeID and diskID
    VC->>DS: SetReplicaRequirement(replicaName, volumeSize)

    DS->>DSC: Reconcile DiskSchedule capacity
    DSC->>DS: Update status.replicas[replicaName]

    alt state == scheduled and size matches
        DS->>RC: status.replicas[replicaName].state: scheduled
        RC->>RC: Create replica instance on disk
    else state == rejected
        DS->>RC: status.replicas[replicaName].state: rejected
        RC->>R: Mark Replica failed
        VC->>R: Delete failed Replica
        RC->>RC: Reconcile Replica deletion
        RC->>DS: SetReplicaRequirement(replicaName, 0)
        DSC->>DS: remove status.replicas[replicaName]
        VC->>VC: Retry Replica creation
    end
```
...
4. If a replica's status is `rejected`:
   - The Replica Controller receives the DiskSchedule CR update event (via `enqueueDiskScheduleChange`), then marks the replica failed.
   - The Volume Controller clear the failed Replica and retry creation from step 1.
   - During failed Replica deletion, the Replica Controller removes the failed scheduling request by calling `SetReplicaRequirement(replicaName, 0)`.
...
sequenceDiagram
    autonumber
    participant BIC as BackingImageController
    participant BI as BackingImageCR
    participant DS as DiskScheduleCR
    participant DSC as DiskScheduleController

    BIC->>BIC: Evaluate disk candidates across DiskScheduleCRs
    BIC->>BI: Add copy to spec.diskFileSpecMap[targetDisk]=fileSpec
    BIC->>DS: SetBackingImageRequirement(backingImageName, imageSize)

    DS->>DSC: Reconcile DiskSchedule capacity for backing image
    DSC->>DS: Update status.backingImages[backingImageName]

    alt state == scheduled
        DS->>BIC: status.backingImages[backingImageName].state: scheduled
        BIC->>BIC: Create backing image copy on assigned disk
    else state == rejected
        DS->>BIC: status.backingImages[backingImageName].state: rejected
        BIC->>DS: SetBackingImageRequirement(backingImageName, 0)
        DSC->>DS: remove status.backingImagesp[backingImageName]
        BIC->>BIC: Retry from step 1 with different disk
    end
```
...
Fon single copy deallocation:

1. When an item removes from `backingImage.Spec.DiskFileSpecMap` in `syncBackingImage`:
   - Get the DiskSchedule CR for the disk
...
2. Collect all disk IDs from `backingImage.Spec.DiskFileSpecMap` keys
3. For each disk ID:
   - Get the DiskSchedule CR for the disk
   - Call `DiskSchedule.SetBackingImageRequirement(backingImage.Name, 0)` to release the allocation
   - Update the DiskSchedule CR
...
3. **Volume condition**: The VolumeController sets `VolumeConditionTypeDiskAllocation` based on `DiskSchedule.Status.Replicas`.

## test plan

### Basic Replica Allocation

1. Given nodes with enough disk
2. Create a volume with a reasonable size
3. Attach the volume to a node
4. Verify that the volume is attached successfully
5. Verify each allocation statuses are successfully recorded in corresponding DiskSchedule CRs
6. Detach and delete the volume
7. After the replicas are deleted, verify that the allocation in each DiskSchedule CRs is eliminated

### Basic Backing Image Allocation

1. Given nodes with enough disk
2. Create a volume with 50GiB replica size
3. Attach the volume to a node
4. Verify that the volume is attached successfully
5. Verify each allocation statuses are successfully recorded in corresponding DiskSchedule CRs
6. Detach and delete the volume
...
1. Given a node with 100GiB disk,
2. Create a volume with 50GiB replica size
3. Create a second volume with 50GiB replica size
4. Verify that both volumes are allocated on the same node
5. Delete one of the volumes
6. Verify that the remaining volume is allocated on the same node
