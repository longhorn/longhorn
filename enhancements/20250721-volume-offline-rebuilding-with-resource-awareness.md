...
- A global setting determines the maximum backoff interval.
...
A global setting determines the max backoff interval

```golang
const(
  ...
  SettingNameOfflineReplicaRebuildingMaxBackoffInerval   = SettingName("offline-replica-rebuilding-max-backoff-interval")
  ...
)
...
SettingDefinitionOfflineReplicaRebuildingMaxBackoffInerval = SettingDefinition{
  DisplayName: "Offline Replica rebuilding Maximum Backoff Interval",
  Description: "In Minutes. The default maximum backoff interval that Longhorn will cancel the volume offline replica rebuilding",
...
```golang
const (
  offlineRebuildingBackoffInitialInterval = 15 * time.Second
  maxOfflineRebuildingRetryCount = 3
)

type offlineRebuildingBackoff struct {
...
func NewVolumeRebuildingController(...) (*VolumeRebuildingController, error) {
  ...
  vbc := &VolumeRebuildingController{
    ...
    // map[volumeName][backupoff, retry]
    offlineRebuildingBackoffMap: make(map[string]*offlineRebuildingBackoff),
    ...
  }
}
...
func (vbc *VolumeRebuildingController) isVolumeReplicasRebuilding(...) bool {
  healthyCount := 0
  for _, mode := range engine.Status.ReplicaModeMap {
    if mode == longhorn.ReplicaModeWO {
      //Remove the volume from backoff map here.
      return true
    }
    ...
  }

  // Initialize the volume backoff and add into the offlilneRebuildBackoffMap

  // Check if it is not in backup off interval and then
  //   Check if it is up to the maximum interval and starts to count the retry for three times.
  //   Check if the retry count is up to the maximum retry counts.
  //     Remove the volume from backoff map if yes.
  //     Go to next backoff interval if no.
  // Re-enqueue the volume after the backoff interval
...
- A worker node goes down during offline rebuilding in the cluster with 3 worker nodes:
  ...
  9. The volume is detached and still degraded after 15 minutes.
