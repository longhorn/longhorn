...
- Formula:
    ...
    $$\text{usableRatio}(disk) = \dfrac{\text{usable}(disk)}{\text{totalCapacity}(disk)}$$
...
| **Hybrid (Weighted Mix)** | - Combines absolute and ratio awareness<br>- Tunable trade-off ($\alpha$) | - More complex and computation<br>- Needs clear default $\alpha$ to avoid confusion  |
