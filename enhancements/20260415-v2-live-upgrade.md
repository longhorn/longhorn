...
#### (1) Allow Instance Manager Automatic Upgrade

- **Name**: `allow-instance-manager-automatic-upgrade`
- **Type**: Boolean
- **Default**: `{"v2":"false"}`
- **Category**: Upgrade
...
#### (2) Instance Manager Upgrade Start Time

- **Name**: `instance-manager-upgrade-start-time`
- **Type**: String (RFC3339 format)
- **Default**: `{"v2":""}` (empty - starts immediately)
- **Category**: Upgrade
...
#### (3) Instance Manager Upgrade Timeout

- **Name**: `instance-manager-upgrade-timeout`
...
| `StartedAt` | string | RFC3339 timestamp when upgrade first entered a timed wait or active execution phase. Used for timeout enforcement. It is not reset between states of the same IMU attempt. |
...
- **Suppression Window**: Plans are created only for attached v2 volumes, and suppression remains active while the volume is attaching or detaching. `Pending` suppresses only when no engines are recorded in the IMU, `RelocatingEngines` suppresses only volumes listed in `IMU.Status.Engines`, and `WaitingForSourceIM`/`RestoringEngines` suppress planned detached replicas unconditionally.
...
- **One timeout window per IMU attempt**, measured from `IMU.Status.StartedAt` and not reset between states. Retrying a failed node resets `StartedAt` for the new attempt.
...
**Uninstall**:
- The uninstall controller disables `allow-instance-manager-automatic-upgrade` before deleting upgrade resources.
...
**Scheduled upgrades**: Start at a specific time via `instance-manager-upgrade-start-time` (projected into `Spec.StartAt` before the cycle begins)
...
**Sharded volumes**: Not supported. The live-upgrade relocation flow requires Replica CRs and a healthy RW replica on the temporary node.
...
The v2 live upgrade capability is introduced as a new CRD-based feature. Offline upgrades from versions earlier than `v1.12.2` remain supported. Longhorn records an unsupported source marker and rejects enabling live upgrade until a subsequent offline upgrade starts from `v1.12.2` or later.
