...
The `default` backup target functions as usual, independent of multiple backup target management. If a volume doesn't have a specified backup target, the `default` backup target will be used.
...
6. Modifying/Deleting the default backup target URL is now allowed.
...
2. Existing backups on the extra backup target B can be synchronized back to the cluster.
...
1. Set up the default backup target and add an extra backup target which has existing backups.
