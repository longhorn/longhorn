...
Users specify the desired backup block size in the storage class as a parameter. This parameter will be used to create the volumes.

#### Configure The Block Size In Volume CR

There's a field to specify the backup block size to create the volume CR. This field is immutable after the CR is created.
...
While creating the volume form a backup, or creating a DR volume, it does not provide the option to adjust the backup block size, and the block size is always read from existing backup.
...
- The backup validator also prohibits modifications to the new field `backupBlockSize`.
...
- Add a new field, `BackupBlockSize`, to the volume spec.
- While Longhorn manager handles volume creation request, it validates the backup block size in the spec, and sets to the new volume CR.
