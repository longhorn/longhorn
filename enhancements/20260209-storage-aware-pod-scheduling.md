...
The scheduler extender is enabled by setting `storage-aware-pod-scheduling` to `true` in Longhorn settings. When enabled, one longhorn-manager pod is elected as the leader to run the extender HTTP service. If the leader fails, another pod takes over via leader election.
...
extenders:
  - urlPrefix: "http://longhorn-scheduler-extender.longhorn-system.svc:9504"
    ...
    httpTimeout: 40s
    managedResources:
      - name: "longhorn.io/storage-aware-scheduling"
        ignoredByScheduler: true
```

Pods that should use the extender must request the `longhorn.io/storage-aware-scheduling` extended resource:

```yaml
resources:
  requests:
    longhorn.io/storage-aware-scheduling: "1"
  limits:
    longhorn.io/storage-aware-scheduling: "1"
```
...
A new Longhorn setting `storage-aware-pod-scheduling` controls whether the scheduler extender is active. When disabled, the HTTP service does not start.
...
5. After returning the filter result, hold the lock and watch the pod until it leaves Pending phase, or 20 seconds have elapsed, whichever comes first. Not Pending means volumes are attached and replicas are scheduled and running. Disk usage on the Longhorn Node CR may not be updated yet, as it gets updated when the node controller reconciles the node after it sees the replica is updated. Based on testing, waiting for the pod to leave Pending phase is sufficient, though edge cases are possible. If the pod hits a SchedulerError condition, release the lock early. Skip this wait for pod restarts (step 3), since no new capacity is consumed and disk usage does not change. If the race condition (issue 1) is not critical for some users — for example, they only need multi-PVC or pod rescheduling support — this step could be made optional via a setting value, allowing the extender to return immediately without holding the lock.

**Future consideration:** If scheduling performance becomes an issue due to slow pod startup causing the extender to wait up to 20 seconds per pod, an alternative approach is possible: watch only until the pod is scheduled (`spec.nodeName` is populated), then reserve the selected node and release the lock. A separate thread watches the reserved node until all volume replicas are present, then unreserves it. This approach requires using `Immediate` volume binding mode because replica IDs must be known at filter time in order to watch for them on the node.
...
- **Goal 1: Race condition**
  ...
  - Deploy a StatefulSet with 16 replicas in parallel mode (`podManagementPolicy: Parallel`), each requesting a 100Gi PVC with `best-effort` data locality and replica count 1. Pods request the `longhorn.io/storage-aware-scheduling` extended resource. Verify that each node receives exactly 4 pods. Repeat 10 times.
  - Deploy the same StatefulSet but without the `longhorn.io/storage-aware-scheduling` extended resource, so it skips the scheduler extender. Confirm that in this case some nodes receive more than 4 pods and some less (kube-scheduler over-provisions nodes because it relies on stale `CSIStorageCapacity` data). Repeat 10 times.
...
- **Goal 2: Pod with multiple PVCs**
  ...
  - Create a pod with 4 PVCs of 100Gi each (`best-effort` data locality, replica count 1), requesting the `longhorn.io/storage-aware-scheduling` extended resource. Confirm that the pod is placed on node 4 (the only node with enough disks to fit all 4 volumes). Repeat 10 times.
  - Create the same pod but without the `longhorn.io/storage-aware-scheduling` extended resource. Confirm that the pod can be scheduled on any node (kube-scheduler is unaware of per-disk capacity).
...
- **Goal 3: Pod rescheduling**
  ...
  - **Case A: Node drain**
    ...
    - Create a StatefulSet with 4 replicas, each requesting a 100Gi PVC (`best-effort` data locality, replica count 1) with the `longhorn.io/storage-aware-scheduling` extended resource. Cordon all nodes except one 400Gi node. Create the StatefulSet — all pods land on that single node. Uncordon all nodes, then drain the node with the StatefulSet. Confirm all pods move to the other 400Gi node (not the 99Gi nodes which cannot fit them). Repeat 3 times.
    - Same setup but without the `longhorn.io/storage-aware-scheduling` extended resource. Drain the node and confirm that pods can go anywhere, including the 99Gi nodes.

  - **Case B: Pod restart**

    Cluster setup: 4 nodes, each with 1 disk of 400Gi.

    - Create a StatefulSet with 16 replicas, each requesting a 100Gi PVC (`best-effort` data locality, replica count 1) with the `longhorn.io/storage-aware-scheduling` extended resource. PVC reclaim policy set to Retain. Record which node each pod was on. Delete the StatefulSet and recreate it. Confirm each pod goes back to the same node as before. Repeat 3 times.
    - Same setup but without the `longhorn.io/storage-aware-scheduling` extended resource. Delete and recreate the StatefulSet. Confirm that pods may go to different nodes than before.
...
This proposal is targeted at volumes with `best-effort` data locality.
...
### kube-scheduler DNS configuration
