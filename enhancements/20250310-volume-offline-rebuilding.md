...
- The CSI attach request during volume offline rebuilding
  1. Lower the volume rebuild VA ticket priority to `1000` (the volume rebuild VA priority needs to be lower than CSI VA priority `900`).
  2. When The CSI attaching volume request is received and the CSI VA ticket is created, the volume rebuild VA ticket will be preempted by the CSI VA ticket.
  3. The volume rebuild controller will remove the volume rebuild VA ticket if there is a CSI VA ticket.
  4. The volume rebuild controller will record the cancellation of the volume offline rebuilding.
  5. The volume will be detached when the volume rebuild VA ticket is removed, then the volume will be attached for the CSI VA ticket.
